-- Photos and videos attached to a bet.
--
-- Runs after 20260905090000_email_auth.sql. Kept separate because the
-- storage half needs rights on `storage.objects` that not every project
-- grants from the SQL editor; if it fails, the account migration before it
-- still stands and the app still runs — only attachments are unavailable.
